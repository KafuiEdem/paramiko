...
def close(ssh_client):
    if ssh_client.get_transport().is_active() == True:
        ...
        ssh_client.close()
#adding send_from_list function
def send_from_list(lists,shell):
    for list in lists:
        send_command(shell=shell,command=list)
        output = show(shell=shell)
        print(output)
#adding send_from_file() 
def send_from_file(shell,location):
    with open(f"{location}.txt",'r') as f:
        file = f.read()
        send_command(shell,file)
    output = show(shell)
    print(output)

if __name__ == '__main__':
    router1 = {'server_ip': '192.168.123.2', 'server_port': '22', 'user':'edem', 'passwd':'cisco'}
